...
MuError MuOfficeLib_create(MuOfficeLib **pMu)
{
	...
	mu = (MuOfficeLib*)Pal_Mem_calloc(1, sizeof(MuOfficeLib));
	if (mu == NULL)
...
fail:
	...
	if (err)
		doc->error(doc->cookie, (MuOfficeDocErrorType)err);
					...
					MuOfficeDoc              **pDoc)
{
	...
	*pDoc = NULL;
	...
	doc = (MuOfficeDoc*)Pal_Mem_calloc(1, sizeof(*doc));
	if (doc == NULL)
		return MuOfficeDocErrorType_NoError;
...
int MuOfficeDoc_providePassword(MuOfficeDoc *doc, const char *password)
{
	...
	len = strlen(password);
	doc->password = (char*)Pal_Mem_malloc(len+1);
				...
				MuOfficePage        **pPage)
{
	...
	*pPage = NULL;
	...
	page = (MuOfficePage*)Pal_Mem_calloc(1, sizeof(*page));
	if (page == NULL)
		return MuError_OOM;
				...
				MuOfficeRender          **pRender)
{
	...
	*pRender = NULL;
	...
	render = (MuOfficeRender*)Pal_Mem_calloc(1, sizeof(*render));
	if (render == NULL)
		return MuError_OOM;
