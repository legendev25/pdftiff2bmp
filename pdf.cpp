...
extern void ShowProgress(double progress);
extern int	ConvertAndSave(char* pszFilename, BYTE *pSource, int width, int height, int bpp, bool mirror);
...
bool pdf::render(const char *export_filename, int from, int to, int zoom, int colormode)
{
    if (!_good) return false;

    //_export_filename = export_filename;
    //int width = std::log10((double)to) + 1;
    ...
    for (int i = from; i <= to; ++i)
    {
        ...
        fz_try(_ctx)
        {
            ...
            pix = fz_new_pixmap_with_bbox(_ctx, fz_device_rgb(_ctx), &bbox, 0, 1);
            ...
            dev = fz_new_draw_device(_ctx, NULL, pix);
            fz_run_page(_ctx, page, dev, &transform, NULL);
            fz_drop_device(_ctx, dev);

			char dstName[1024];
			sprintf(dstName, "%s(%d).bmp", export_filename, i);

            // Save the pixmap to a file.
			ConvertAndSave(dstName, pix->samples, pix->w, pix->h, colormode, true);

            // Clean up.
            fz_drop_pixmap(_ctx, pix);
            fz_drop_page(_ctx, page);

			ShowProgress((double)(i - from + 1) / (double)(to - from + 1) * 100.0);
        }
        fz_catch(_ctx)
        {
            return false;
